...
4. 문자열 여러개 저장하기 <u>*포인터배열 vs 이차원배열*</u>

    * 포인터 배열 : 문자열 상수 사용, 텍스트 세그먼트의 주소 저장 --> 원소 접근 가능, 변경 불가능.  
    * 이차원 배열 : char형 배열 공간 사용 --> 원소 접근 가능, 변경 불가능. 단, 비효율적.
...
* 함수 매크로  
    ...
    * '\'역슬래시를 사용하면 매크로 정의에서 한 줄을 연장할 수 있다.(전처리기는 컴파일러와 달리 한줄단위로 읽기 때문이다.)
...
1. 분할 컴파일의 필요성
    * 비슷한 함수들을 모듈로 묶어 후에 모듈의 재사용을 위해서.
    * 모든 코드를 하나의 소스파일에 넣는 것보다 여러 소스파일로 분할하여 컴파일하는 것이 유지보수 측면에서도, 용량의 효율성 측면에서도 더 좋다.


2. 분할 컴파일의 원리 
    > 함수의 선언부와 구현부를 분리하여 모듈화한다.

    > 전처리기는 include지시자에 의해 소스파일에 헤더파일 소스(함수 선언, 구조체 선언 등)를 추가한다.

    > 컴파일러는 구현 파일을 포함한, 모든 소스파일을 컴파일한다.(__헤더파일에 구현부를 넣을 필요가 없는 이유__)
    >   > 다시말해 __전방 선언__ 을 통해 컴파일러가 함수 정의 없이도 컴파일을 가능케 하고, 함수 정의는 다른 소스파일에 구현하여 컴파일러가 이 소스파일을 컴파일 하면 문제가 없어진다.


3. 궁금했던 점 해결하기

* 선언부와 구현부를 분리하는 이유?
    * --> 각각의 소스파일의 용량을 줄여 효율적인 분할 컴파일을 꾀하는 것인데 헤더파일에 함수 정의를 포함하면 전처리기는 매번마다 소스에 헤더파일 코드를 추가하므로 소스파일의 용량이 커진다.
    * --> 복잡한 로직에 대응하기 좋다.


* 구현파일에도 헤더파일을 추가하는 이유?
    * --> 구현파일에서도 함수 서로가 서로를 호출하는 경우에, 서로가 서로의 존재를 알고 있어야하므로 헤더파일을 include한다.
    

### 동적 메모리 할당  

1. 동적 메모리 할당 원리  
    > 1. 운영체제는 프로그램을 실행할 때, 4개의 공간을 할당한다. 그 중 동적 메모리를 할당할 공간은 heap공간이다.  

    > 2. 라이브러리 함수를 이용하여 운영체제에게 할당받을 메모리를 요청한다.

    > 3. 충분한 메모리가 존재하면 메모리를 할당한다.

    > 4. 메모리를 사용 후 운영체제에게 메모리를 반납한다.  

2. 동적 메모리 할당 함수  

        (void *)malloc(int size_t)
        반환값 : 할당받은 메모리의 주소를 반환한다.
        매개변수 : 할당받고 싶은 바이트의 수이다.(바이트 단위로 메모리를 할당한다.)


        (void *)calloc(항목의 개수, 항목의 크기)
        매개변수 : 항목 단위로 메모리를 할당한다.

        free() //할당받은 메모리를 반납한다.
        인수 : 할당 받은 메모리의 주소

        realloc() //기존의 동적 메모리의 크기를 변경하여 다시 할당한다.
        <예시>
        int *p;
...
4. 동적으로 2차원 배열 생성하기
    ...
    > 이중 포인터를 활용한다.
        ...
        int **q;
        q= (int **)malloc(rows * sizeof(__int *__)); //자료형이 int *형인 배열 즉, 포인터 배열로 할당한다.
        for(i = 0; i<rows; i++)
            q[i] = (int *)malloc(columns * sizeof(int));    //각 원소(int 형 포인터)에게 또 다시 int형 동적 배열을 할당한다.
